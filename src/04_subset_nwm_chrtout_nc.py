...
# Read NWM Parameters
gdf_nwm3_reaches_az_lcc = pyogrio.read_dataframe(param_nwm3.gp_nwm_reaches_az_lcc)
gdf_nwm3_catchments_az_lcc = pyogrio.read_dataframe(param_nwm3.gp_nwm_catchments_az_lcc)
gdf_nwm3_waterbodies_az_lcc = pyogrio.read_dataframe(param_nwm3.gp_nwm_waterbodies_az_lcc)
...
ds = nwm.s3_nwm3_zarr_bucket('CONUS/zarr/chrtout.zarr')
ds = ds[var]

var_dict = {
            'q_lateral':{'varname':'q_lateral',
                      'unit_conversion':1, # m3 s-1 --> m3 s-1
                      'new_unit':'m3 s-1'},
            'qSfcLatRunoff':{'varname':'qSfcLatRunoff',
                        'unit_conversion':1, # m3 s-1 --> m3 s-1
                        'new_unit':'m3 s-1'},
            'qBucket':{'varname':'qBucket',
                        'unit_conversion':1, # m3 s-1 --> m3 s-1
                        'new_unit':'m3 s-1'},
            'streamflow':{'varname':'streamflow',
                      'unit_conversion':1, # m3 s-1 --> m3 s-1
                      'new_unit':'m3 s-1'}, 
                    }
...
for dt in date_range:
    ...
    savedir = os.path.join(param_nwm3.out_dir,os.path.basename(__file__).split('.')[0],'chrtout','nc','hourly',var)
